 ...
 */
export function liveStatus(lastHeartbeatAtMs: number, now: number = Date.now()): LiveStatus {
 ...
 */
export function liveStatusSql(now: number = Date.now()): SQL {
  return sql`CASE
 ...
 */
export function liveStatusWhere(status: LiveStatus, now: number = Date.now()): SQL {
